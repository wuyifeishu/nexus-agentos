...
from .validation import ErrorCategory, ToolErrorClassifier
        ...
        if max_batch_size is not None:
            ...
            for i in range(0, len(tool_calls), max_batch_size):
                ...
                batch_results = await asyncio.gather(
                    *[self.execute(tool, args, timeout=timeout_per_tool) for tool, args in batch]
                )
            ...
            # 并发执行所有工具
            tasks = [
                self.execute(tool, args, timeout=timeout_per_tool) for tool, args in tool_calls
            ]
            return await asyncio.gather(*tasks)

    def get_metrics(self, tool_name: str | None = None) -> dict[str, ExecutionMetrics]:
        """获取性能指标。"""
        if tool_name:
            return {tool_name: self._metrics.get(tool_name)}
        return self._metrics.copy()

    def get_circuit_breaker_state(self, tool_name: str) -> CircuitBreakerState | None:
        """获取熔断器状态。"""
...
class SmartRetryExecutor:
    ...
    async def execute_with_retry(
        ...
        arguments: dict[str, Any],
        call_id: str | None = None,
        base_executor: AsyncToolExecutor | None = None,
    ) -> ToolResult:
        """
        带智能重试的工具执行。
        ...
        for attempt in range(self.max_retries + 1):
            ...
            result = await base_executor.execute(
                tool=tool, arguments=arguments, call_id=f"{call_id}_attempt{attempt}"
            )
        ...
        return last_result or ToolResult.fail(
            call_id=call_id, error="Execution failed after retries"
        )


# 便捷函数
async def execute_tool_with_retry(
    tool: BaseTool, arguments: dict[str, Any], max_retries: int = 3, call_id: str | None = None
) -> ToolResult:
    """
    带重试的工具执行便捷函数。
...
async def execute_tools_concurrently(
    ...
    return await executor.execute_batch(tool_calls=tool_calls, timeout_per_tool=timeout_per_tool)
