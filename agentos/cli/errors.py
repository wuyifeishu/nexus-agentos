...
def no_provider_configured():
    ...
    print()
    print(f"    AgentOS 需要至少一个 LLM 服务商的 API Key 才能运行。")
    print()
    print(f"    支持的服务商：")
...
def single_provider_failed(provider_name: str, error: str = ""):
    ...
    print(f"    2. 网络不通 → 检查代理/防火墙设置")
    print(f"    3. 余额不足 → 检查服务商账户余额")
    print()
    print(f"    或切换到其他 Provider：")
    print(f"    export OPENAI_API_KEY=sk-xxx   # 备用")
    print()
    sys.exit(1)
...
def all_providers_failed(failures: list[tuple[str, str]]):
    ...
    print(f"    2. 检查网络连接和代理设置")
    print(f"    3. 检查各服务商账户余额")
    print()
    sys.exit(1)
...
def feature_disabled(feature: str, mode: str = "当前运行模式"):
    ...
    print()
    print(f"    如需完整功能，请在桌面端运行 AgentOS。")
    print()
...
def import_error(module: str):
    ...
    print(f"    或：pip install nexus-agentos[all]")
    print()
